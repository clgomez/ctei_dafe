...
export class ProyectosTutComponent implements OnInit {
  currentUser: User | null = null;
   ...
   public usuarioTutor: Usuario | null;
  ...
  ngOnInit() {
    ...
    this.cargarUsuarioTutor();
    this.cargarInscripciones();

  }

   cargarUsuarioTutor(): void{
    this.activatedRoute.params.subscribe(params => {
      let id = params['idusuariotutor']
      if(id){
        this.usuarioService.getUsuarioPorId(id).subscribe( (usuarioTutor) =>
          {this.usuarioTutor = usuarioTutor;
           
            console.log('usuario tutor',this.usuarioTutor)
